...
pub fn extract_trace_id(headers: &http::HeaderMap) -> phprt_core::TraceId {
            ...
            if parts.len() >= 3 && parts[1].len() == 32 {
                ...
                if let (Ok(high), Ok(low)) = (
                    ...
                    if let Ok(uuid) = uuid::Uuid::from_slice(&bytes) {
                        return phprt_core::TraceId::from_uuid(uuid);
                    }
                }
            }
        }
    }
    phprt_core::TraceId::new()
}
