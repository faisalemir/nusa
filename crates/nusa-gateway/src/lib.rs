...
use nusa_core::{
    ...
    TenantRegistry, TaskManager, TenantRateLimiter,
};
...
pub fn app(
    ...
    let state = AppState {
        engine, plugins, circuit_breaker, health_state, backpressure,
        resource_guard: Arc::new(resource_guard), tenants, tasks,
        rate_limiter, tenant_cb, ws_manager, sse_manager, static_handler, metrics,
    };
    ...
    Router::new()
        ...
        .route("/api/tasks/{task_id}/status", axum::routing::get(task_status_handler))
        // Static files (Blueprint 6 E1) — stub route for now
        .route("/static/{*path}", axum::routing::get(|_path: axum::extract::Path<String>| async move {
            (StatusCode::NOT_IMPLEMENTED, "Static file serving not yet implemented")
        }))
...
async fn ws_upgrade_stub_handler() -> (StatusCode, &'static str) {
    (StatusCode::NOT_IMPLEMENTED, "WebSocket support requires axum 'ws' feature.")
}

/// SSE handler route (Blueprint 6 F2).
async fn sse_handler(
    State(state): State<AppState>,
) -> axum::response::Sse<impl futures::stream::Stream<Item = Result<axum::response::sse::Event, std::convert::Infallible>> + use<>> {
    axum::response::Sse::new(state.sse_manager.stream())
        .keep_alive(axum::response::sse::KeepAlive::new()
            .interval(Duration::from_secs(15))
            .text("keep-alive"))
}

/// Static file handler route (Blueprint 6 E1).
#[allow(dead_code)]
async fn static_file_handler(
    ...
    let stub_req = Request::builder()
        .uri(format!("/{}", path))
        .body(Body::empty())
        .expect("valid");

    match state.static_handler.serve(&path, &stub_req).await {
        Some(resp) => resp,
        None => {
            Response::builder()
...
async fn handler(
    ...
    match with_timeout(state.resource_guard.request_timeout_ms, state.engine.execute(ctx)).await {
        Ok(res) => {
            ...
            drop(permit);
            state.metrics.request_duration_ms.record(start.elapsed().as_secs_f64() * 1000.0);
